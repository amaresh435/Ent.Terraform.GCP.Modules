resource "google_container_node_pool" "admin" {
  ...
  name              = join("-", [local.resource_prefix, "admin", "nodepool"])
  location          = var.region
  ...
  autoscaling {
    total_mix_node_count = var.gke_autoscaling_config_admin_node.min_node_count
    total_max_node_count = var.gke_autoscaling_config_admin_node.max_node_count
